...
export const createHtmlCell = (html = '', className = '') => {
    ...
    // Intentar detectar un patrón simple: <tag class="...">innerText</tag>
    // Esto evita usar innerHTML pero permite badges y tags controlados.
    // Nota: no pretende ser un parser HTML general, solo soporta el uso
    // controlado presente en la base de código (badges, spans simples).
    const simpleTagRe = /^<([a-zA-Z0-9]+)(?:\s+class="([^"]*)")?[^>]*>([\s\S]*?)<\/\1>$/
    ...
    if (m) {
        ...
        // inner may contain text (expected). Insert as textContent to avoid XSS.
        node.textContent = inner.replaceAll('\n', ' ').trim()
        el.appendChild(node)
        return el
    }

    // Fallback: treat as plain text (escaped earlier if needed)

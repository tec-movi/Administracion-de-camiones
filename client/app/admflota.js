import { requestJson, setAlert, renderSidebar, API_BASE, createCell, createActionButton } from "./shared.js"

// Endpoints backend usados por el módulo de administración de flota.
const API_TRUCKS_URL = `${API_BASE}/trucks`
const API_AVAILABLE_DRIVERS_URL = `${API_BASE}/users/available-drivers`
const API_REGISTER_URL = `${API_BASE}/sessions/register`
const API_ASSIGNMENTS_URL = `${API_BASE}/assignments`
...
const loadTrucks = async () => {
  ...
  trucks.forEach((truck) => {
    ...
    const row = document.createElement('tr')
    row.appendChild(createCell('td', truck.plate_number || '-'))
    row.appendChild(createCell('td', truck.brand || '-'))
    row.appendChild(createCell('td', truck.model || '-'))
    row.appendChild(createCell('td', truck.year || '-'))
    row.appendChild(createCell('td', driverName))

    const actionTd = document.createElement('td')
    if (!activeAssignment) {
      const btnAssign = document.createElement('button')
      btnAssign.type = 'button'
      btnAssign.className = 'btn btn-primary btn-sm js-open-assign-modal'
      btnAssign.dataset.truckId = String(truck.id)
      btnAssign.textContent = 'Asignar conductor'
      actionTd.appendChild(btnAssign)
    } else {
      const btnUnassign = document.createElement('button')
      btnUnassign.type = 'button'
      btnUnassign.className = 'btn btn-danger btn-sm js-unassign-btn'
      btnUnassign.dataset.assignmentId = String(activeAssignment.id)
      btnUnassign.textContent = 'Desvincular'
      actionTd.appendChild(btnUnassign)
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const assignDriverModal = window.bootstrap?.Modal.getOrCreateInstance(assignDriverModalEl)
    ...
    assignDriverSelect.textContent = ''
    const placeholder = document.createElement('option')
    placeholder.value = ''
    placeholder.disabled = true
    placeholder.selected = true
    placeholder.textContent = 'Selecciona conductor'
  ...
  const openAssignModal = async (truckId) => {
    ...
    assignDriverSelect.appendChild(loadingOpt)

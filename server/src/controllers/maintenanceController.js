...
const startMaintenance = async (req, res) => {
  try {
    ...
    const bodyMaintenanceId = req.body?.maintenance_id
    const targetMaintenanceId = maintenance_id || bodyMaintenanceId
    await maintenanceService.startMaintenance(targetMaintenanceId)
...
const completeMaintenance = async (req, res) => {
  try {
    const { maintenance_id } = req.params
    const { truck_id, maintenance_mileage, description } = req.body
    await maintenanceService.finishMaintenance(maintenance_id, truck_id, maintenance_mileage, description)
